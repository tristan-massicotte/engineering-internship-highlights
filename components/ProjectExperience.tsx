...
function SupplierReliability({range}:{range:number}){
  ...
  return <div className="supplier-ranking"><div className="ranking-controls"><span>Rank by</span>{(['score','quality','onTime','delay'] as const).map(option=><button className={sort===option?'active':''} onClick={()=>setSort(option)} key={option}>{option==='onTime'?'On-time':option}</button>)}</div><div className="ranking-table"><div className="ranking-row ranking-head"><span>Rank</span><span>Supplier</span><span>Reliability score</span><span>Non-conformity returns</span><span>On-time delivery</span><span>Average delay</span></div>{ranked.map((supplier,index)=><div className="ranking-row" key={supplier.name}><strong>0{index+1}</strong><b>{supplier.name}</b><span className="score-cell" data-label="Reliability"><i><em style={{width:`${supplier.score}%`}}/></i>{supplier.score}/100</span><span data-label="Returns">{supplier.nonConformity.toFixed(1)}%</span><span data-label="On-time">{supplier.onTime}%</span><span data-label="Average delay">{Math.round(supplier.averageDelay)} days</span></div>)}</div></div>;
}
...
function ProductLines({range}:{range:number}){
  ...
  return <div className="product-lines"><div className="product-line-legend">{productSeries.map(product=><span className="active" key={product.name}><i style={{background:product.color}}/>{product.name}</span>)}</div><div className="mobile-chart-hint" aria-hidden="true">Swipe chart</div><div className="mobile-chart-scroll"><svg viewBox="0 0 680 325" role="img" aria-label={`Monthly assembled product volumes over the selected ${range} month horizon`}><g className="product-grid">{[0,40,80,120,160].map(value=><g key={value}><line x1="72" x2="640" y1={y(value)} y2={y(value)}/><text x="60" y={y(value)+3} textAnchor="end">{value}</text></g>)}</g><text x="19" y="145" transform="rotate(-90 19 145)" className="axis-title">UNITS ASSEMBLED</text>{productSeries.map(product=>{const values=product.values.slice(start);return <g className="product-series visible" style={{'--series-color':product.color} as React.CSSProperties} key={product.name}><polyline points={values.map((value,pointIndex)=>`${x(pointIndex)},${y(value)}`).join(' ')}/>{values.map((value,pointIndex)=><circle key={pointIndex} cx={x(pointIndex)} cy={y(value)} r="3"/>)}</g>})}<g className="product-months">{months.map((month,index)=><text key={month} x={x(index)} y="300" textAnchor="middle">{month}</text>)}</g></svg></div><div className="product-chart-footer"><span><i/>Monthly output · {range} month horizon</span><b>{totals[0].name} <strong>{totals[0].total.toLocaleString()}</strong> units assembled</b></div></div>;
}
